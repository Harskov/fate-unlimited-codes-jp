...
| 20 | `data_01` | data | 0x003D3000 | 0x004E2000 | 0x10f000 | 0 | — |
| 21 | `rodata_str` | rodata | 0x004E2000 | 0x00509100 | 0x27100 | 0 | — |
| 22 | `main_bss` | bss | 0x00509100 | 0x00536280 | 0x2d180 | 0 | — |
...
**`sdk_dma_graph`** (sdk, 0x00100230–0x00103F60)
...
**`sdk_kernel`** (sdk, 0x00103F60–0x00107800)
...
- boundary moved to the nearest 16-byte-aligned function start below the evidence edge: gen_splat_yaml.py writes subalign: 16, so an 8-aligned boundary pads the image (run friction F-4)
...
**`game_00`** (game, 0x00158030–0x002A5FC0)
...
**`game_render`** (game, 0x002A5FC0–0x002E6040)
...
**`sdk_cdvd`** (sdk, 0x003536B0–0x00353FF0)
...
**`game_02`** (game, 0x00353FF0–0x0036EB30)
...
- no function references this range directly: zero %hi/%lo/%gp_rel operands naming D_0038xxxx/D_0039xxxx/D_003Axxxx in asm/main.s — consistent with a DMA-uploaded blob (VU microcode / packet tables) reached through pointers held in data_01
- referencing_functions: 0 — none
...
- referencing_functions: 589 (first func_00100388, last func_00158A48)
...
- referencing_functions: 638 (first func_00158B98, last func_003844F0)
...
- referencing_functions: 1194 (first func_00100230, last func_00384130)

**`main_bss`** (bss, 0x00509100–0x00536280)
- LOAD segment memsz 0x00436280 - filesz 0x00409100 = 0x0002D180 (config/target.json program_headers[0])
- named main_bss, not bss: splat writes no asm/data/<name>.bss.s when the subsegment is named 'bss' while still emitting its .o in the ld script (run friction F-3)
