# Friction Report — 2026-09-05-004-split-baseline

**Job folder:** `2026-09-05-004-split-baseline`  
**Log:** `friction-log.jsonl` (2 entries; opened 2026-09-05T20:23:11Z, exported 2026-09-05T20:29:12Z)  
...
- **Stage:** Stage E EXECUTE | **Tool:** `configure.py` | **Logged:** 2026-09-05T20:28:25Z
- **Observed (fact):** ld: undefined reference to D_00509100, D_536280, D_00510470, D_40000, D_00509380, D_FFFFFF ... (11) on the one-segment baseline; after adding a c subsegment at 0x77B80 the second file (main_b) referenced .L0010AF40 defined in main.s. configure.py now appends 'NAME = 0xADDR;' lines to config/undefined_syms.txt for address-named undefined symbols and relinks; the image then matches byte for byte.
- **Expected:** splat's undefined_syms_auto.txt covers every referenced symbol
- **Operator hypothesis — UNVERIFIED, verify against the artifacts before building on it:** spimdisasm treats data inside a region typed asm as instructions, so symbols that fall mid-instruction or in misdetected functions get no label; segment-map (S4) typing data regions as data will remove most of these. Unverified.
- **Artifact:** `../../config/undefined_syms.txt` (exists)
...
- **Stage:** Cross-cutting / environment | **Tool:** `ninja` | **Logged:** 2026-09-05T20:28:25Z
- **Observed (fact):** ninja: error: remove(.ninja_lock): Operation not permitted, printed after [4/4] ld; the ELF exists. configure.py treats that error as success when the ELF was produced and no subcommand failed.
- **Expected:** ninja exit 0 after a complete build
- **Repro:** `cd targets/fate-unlimited-codes-jp && tools/venja/bin/ninja`
...
```json
{
  "job": "2026-09-05-004-split-baseline",
  "exported": "2026-09-05T20:29:12Z",
  ...
  "entries": [
    {
      ...
      "severity": "minor",
      "category": "tool-bug",
      "title": "First link of the split image failed on 11 address-named symbols spimdisasm referenced but placed nowhere, plus one .L label referenced across a file split",
      "tool": "configure.py"
    },
    {
      "id": "F-2",
      ...
      "title": "ninja cannot remove .ninja_lock on the mount and exits non-zero after a complete build",
      "tool": "ninja"
    }
  ]
}
