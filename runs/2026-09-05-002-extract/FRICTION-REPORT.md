# Friction Report — 2026-09-05-002-extract

**Job folder:** `2026-09-05-002-extract`  
**Log:** `friction-log.jsonl` (1 entry; opened 2026-09-05T19:45:17Z, exported 2026-09-05T19:46:18Z)  
...
- **Stage:** Stage E EXECUTE | **Tool:** `extract_elf.py` | **Logged:** 2026-09-05T19:46:17Z
- **Observed (fact):** First run: irx_modules = ['libmc: too old release of mcman.irx', ...], sce_identifier_count 3, string_count 3788 (min length 6). Fixed in the same build: IRX from the ISO MODULES\ directory, sce* by substring, min string length 4.
- **Expected:** IRX list = disc MODULES\ contents; sce* identifiers in the hundreds
- **Operator hypothesis — UNVERIFIED, verify against the artifacts before building on it:** regexes anchored to whole strings; fixed
- **Repro:** `python3 skills/dps2-toolkit/scripts/extract_elf.py --repo targets/fate-unlimited-codes-jp --iso "Fate-Unlimited Codes (Japan).iso" --force`
...
```json
{
  "job": "2026-09-05-002-extract",
  "exported": "2026-09-05T19:46:18Z",
  ...
  "entries": [
    {
      ...
      "severity": "papercut",
      "category": "tool-bug",
      "title": "extract_elf.py first pass listed IRX names from error strings and found 3 sce* identifiers instead of the disc's MODULES list and the design's sce* counts",
      "tool": "extract_elf.py"
    }
  ]
}
